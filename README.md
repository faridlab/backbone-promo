...
```bash
# Generate everything
backbone schema generate promo --target all

# Or generate specific targets
backbone schema generate promo --target proto,rust,sql
backbone schema generate promo --target handler,grpc,cqrs
backbone schema generate promo --target repository,events
```
...
let module = PromoModule::builder()
    .with_database(pool)
    .build()?;
...
2. Run generator: `backbone schema generate promo --target all`
...
2. Generate migration: `backbone migration alter {Entity} promo -d "description"`
3. Regenerate code: `backbone schema generate promo --target all`
4. Run migrations
...
```bash
# Run all tests
cargo test --package backbone-promo

# Run with database
DATABASE_URL=postgresql://... cargo test --package backbone-promo
```
