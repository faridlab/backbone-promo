...
//!   * one ACTIVE claim per cart is DB-backed (a direct second `claimed` row for the same cart
//!     surfaces as 23505, not a service-side check);
...
//!
//! The module is tenant-agnostic (ADR-0029): row scoping is the composing service's tenancy
//! decorator, and every probe keys its fixtures on fresh random ids (carts, coupons, sources)
...
fn claim_req(cart: Uuid, code: &str) -> PromoCodeClaimRequest {
...
async fn claim_rows(pool: &sqlx::PgPool, cart: Uuid) -> Vec<(Uuid, String)> {
    let rows = sqlx::query(
        r#"SELECT id, status::text FROM promo.coupon_claims
           WHERE cart_ref_type = 'storefront_cart' AND cart_ref_id = $1
           ORDER BY claimed_at"#,
    )
    .bind(cart)
    .fetch_all(pool)
...
async fn fixture(pool: &sqlx::PgPool, max_use: Option<i32>) -> Uuid {
    let item = Uuid::new_v4();
    let rule_id = pct_rule(pool, item, 0, "10").await;
    coupon(pool, "CLAIMABLE", rule_id, max_use).await
}
...
async fn claim_adjudicates_server_side_and_persists() {
    ...
    let coupon_id = fixture(&pool, Some(5)).await;
    let cart = Uuid::new_v4();
    let sink = RecSink::default();

    let out = svc
        .claim_promo_code(&claim_req(cart, "  claimable  "), &sink)
        .await
        .expect("claim");
    ...
    let rows = svc.claims_for_cart("storefront_cart", cart).await.expect("history");
...
async fn claim_is_idempotent_per_cart_and_code() {
    ...
    let _wide = isolated_wide_tables(&pool).await;
    let svc = PromoWriteService::new(pool.clone());
    fixture(&pool, Some(2)).await;
    let cart = Uuid::new_v4();
    let sink = RecSink::default();

    let first = svc.claim_promo_code(&claim_req(cart, "CLAIMABLE"), &sink).await.unwrap();
    // The same code typed differently is still the same claim.
    let second = svc.claim_promo_code(&claim_req(cart, "claimable"), &sink).await.unwrap();
    assert_eq!(second.claim_id, first.claim_id);
    assert!(second.already);
    assert_eq!(claim_rows(&pool, cart).await.len(), 1);
    assert_eq!(sink.count_matching(|e| matches!(e, PromoEvent::PromoCodeClaimed(_))), 1);
}
...
async fn capped_last_unit_holder_replays_its_own_claim() {
    ...
    let _wide = isolated_wide_tables(&pool).await;
    let svc = PromoWriteService::new(pool.clone());
    let coupon_id = fixture(&pool, Some(1)).await;
    let holder = Uuid::new_v4();
    let sink = RecSink::default();

    let first = svc
        .claim_promo_code(&claim_req(holder, "CLAIMABLE"), &sink)
        .await
    ...
    let rows = claim_rows(&pool, holder).await;
    assert_eq!(rows.len(), 1);
    ...
    let other = svc
        .claim_promo_code(&claim_req(Uuid::new_v4(), "CLAIMABLE"), &LoggingSink)
        .await
        .unwrap_err();
...
async fn claim_refusals_are_uniform() {
    ...
    // Unknown code.
    let unknown = svc
        .claim_promo_code(&claim_req(Uuid::new_v4(), "NOSUCHCODE"), &LoggingSink)
        .await
        .unwrap_err();

    // Inactive coupon.
    let inactive_id = coupon(&pool, "SLEEPY", rule_id, None).await;
    sqlx::query("UPDATE promo.coupon_codes SET status = 'inactive' WHERE id = $1")
        ...
        .unwrap();
    let inactive = svc
        .claim_promo_code(&claim_req(Uuid::new_v4(), "SLEEPY"), &LoggingSink)
        .await
        .unwrap_err();

    // Out-of-window coupon (window already closed).
    let expired_id = coupon(&pool, "PASTDUE", rule_id, None).await;
    sqlx::query("UPDATE promo.coupon_codes SET valid_upto = $1 WHERE id = $2")
        ...
        .unwrap();
    let expired = svc
        .claim_promo_code(&claim_req(Uuid::new_v4(), "PASTDUE"), &LoggingSink)
        .await
        .unwrap_err();

    // Exhausted at rest: max_use fully burned already.
    let burned_id = coupon(&pool, "ALLGONE", rule_id, Some(1)).await;
    sqlx::query("UPDATE promo.coupon_codes SET used_count = 1 WHERE id = $1")
        ...
        .unwrap();
    let exhausted = svc
        .claim_promo_code(&claim_req(Uuid::new_v4(), "ALLGONE"), &LoggingSink)
        .await
        .unwrap_err();
    ...
    coupon(&pool, "FIRSTPICK", rule_id, None).await;
    coupon(&pool, "SECONDPICK", rule_id, None).await;
    let occupied_cart = Uuid::new_v4();
    svc.claim_promo_code(&claim_req(occupied_cart, "FIRSTPICK"), &LoggingSink)
        .await
        .unwrap();
    let occupied = svc
        .claim_promo_code(&claim_req(occupied_cart, "SECONDPICK"), &LoggingSink)
        .await
        .unwrap_err();
    ...
    for racer in 0..8u64 {
        ...
        handles.push(set.spawn_local(async move {
            ...
            for attempt in 1..=10u64 {
                match svc
                    .claim_promo_code(&claim_req(cart, "CLAIMABLE"), &LoggingSink)
                    .await
                {
...
async fn release_frees_headroom_and_the_cart_slot() {
    ...
    svc.claim_promo_code(&claim_req(cart1, "CLAIMABLE"), &LoggingSink).await.unwrap();
    svc.claim_promo_code(&claim_req(cart2, "CLAIMABLE"), &LoggingSink).await.unwrap();
    let refused = svc
        .claim_promo_code(&claim_req(cart3, "CLAIMABLE"), &LoggingSink)
        .await
        .unwrap_err();
    ...
    let sink = RecSink::default();
    let released = svc
        .release_promo_claim("storefront_cart", cart1, now(), &sink)
        .await
        .unwrap();
    ...
    svc.claim_promo_code(&claim_req(cart3, "CLAIMABLE"), &LoggingSink).await.unwrap();

    // And the freed SLOT lets cart1 claim a different code outright.
    let item = Uuid::new_v4();
    let rule_id = pct_rule(&pool, item, 0, "10").await;
    coupon(&pool, "OTHERCODE", rule_id, None).await;
    svc.claim_promo_code(&claim_req(cart1, "OTHERCODE"), &LoggingSink).await.unwrap();

    let rows1 = claim_rows(&pool, cart1).await;
...
async fn burn_settles_the_same_ref_claim() {
    ...
    let _wide = isolated_wide_tables(&pool).await;
    let svc = PromoWriteService::new(pool.clone());
    let coupon_id = fixture(&pool, Some(3)).await;
    let cart = Uuid::new_v4();

    let out = svc.claim_promo_code(&claim_req(cart, "CLAIMABLE"), &LoggingSink).await.unwrap();

    let rule = svc
        .commit_coupon_redemption(coupon_id, "storefront_cart", cart, &LoggingSink)
        .await
        .expect("burn");
    assert_eq!(rule, out.pricing_rule_id);
    assert_eq!(used_count(&pool, coupon_id).await, 1);

    let rows = svc.claims_for_cart("storefront_cart", cart).await.unwrap();
    assert_eq!(rows.len(), 1);
    ...
    svc.commit_coupon_redemption(coupon_id, "storefront_cart", cart, &LoggingSink)
        .await
        .expect("replay");
    assert_eq!(used_count(&pool, coupon_id).await, 1);
    let rows = claim_rows(&pool, cart).await;
    assert_eq!(rows.len(), 1);
...
async fn burn_without_claim_is_unchanged() {
    ...
    let rule = svc
        .commit_coupon_redemption(coupon_id, "sales_order", order, &LoggingSink)
        .await
        .expect("burn");
    assert!(!rule.is_nil());
    assert_eq!(used_count(&pool, coupon_id).await, 1);
    ...
    let claims = sqlx::query_scalar::<_, i64>(
        "SELECT COUNT(*) FROM promo.coupon_claims WHERE coupon_id = $1",
    )
    .bind(coupon_id)
    .fetch_one(&pool)
    .await
...
// Case-folded code uniqueness is NOT a module-owned fact anymore (ADR-0029): the code unique is
// org-scoped and decorator-declared by the composing service's tenancy decorator, so the bare
// module schema carries no code unique of its own. The former cross-writer uniqueness leg retired
// in favor of the posture probe — see loyalty_fence_probes.rs.
...
async fn one_active_claim_per_cart_is_db_backed() {
    ...
    let item = Uuid::new_v4();
    let rule_id = pct_rule(&pool, item, 0, "10").await;
    let coupon_id = coupon(&pool, "ONEPERTROLLEY", rule_id, None).await;
    let cart = Uuid::new_v4();

    sqlx::query(
        r#"INSERT INTO promo.coupon_claims
               (cart_ref_type, cart_ref_id, coupon_id, code, pricing_rule_id, claimed_at)
           VALUES ('storefront_cart', $1, $2, 'ONEPERTROLLEY', $3, $4)"#,
    )
    .bind(cart)
    .bind(coupon_id)
    .bind(rule_id)
...
// ---- the claims table's tenancy posture ---------------------------------------------------------
//
// The module ships no row fence (ADR-0029): row scoping is the composing service's tenancy
// decorator. The former RLS-fence leg retired in favor of the posture probe — see
// loyalty_fence_probes.rs.
...
async fn malformed_refs_and_empty_code_are_refused_loudly_or_uniformly() {
    ...
    let _wide = isolated_wide_tables(&pool).await;
    let svc = PromoWriteService::new(pool.clone());
    fixture(&pool, None).await;

    let mut req = claim_req(Uuid::new_v4(), "CLAIMABLE");
