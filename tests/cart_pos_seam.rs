...
impl CartPricingPort for PromoCartAdapter {
    async fn price_cart(&self, req: &CartPriceRequest) -> Result<PricedCart, CartPricingError> {
        // POS's post-strip `CartPriceRequest` carries no tenant axis (ADR-0029): the ticket is
        // priced under whatever org scope the composing service bound around this port call.
        let q = CartQuery {
...
async fn open_session(pool: &sqlx::PgPool, profile: Uuid) -> Uuid {
    sqlx::query_scalar::<_, Uuid>(
        r#"INSERT INTO pos.pos_opening_entries (pos_profile_id, cashier_party_id, opened_at, status)
           VALUES ($1,$2, now(), 'open'::pos_session_status) RETURNING id"#,
    )
    .bind(profile)
    .bind(Uuid::new_v4())
    .fetch_one(pool)
...
async fn seed_profile(pool: &sqlx::PgPool, profile: Uuid, template: Uuid) {
    sqlx::query(
        r#"INSERT INTO pos.pos_profiles (id, name, tax_template_ids)
           VALUES ($1, 'seam till', $2::jsonb)"#,
    )
    .bind(profile)
...
async fn cpseam1_cart_discounts_land_on_a_real_ticket() {
    ...
    seed_profile(&pool, profile, Uuid::new_v4()).await;
    let (item_a, item_b) = (Uuid::new_v4(), Uuid::new_v4());
    let session = open_session(&pool, profile).await;
...
async fn cpseam2_free_item_lands_on_the_ticket() {
    ...
    let ticket = NewCartSale {
        pos_profile_id: profile, opening_entry_id: session, branch_id: None,
