...
/// `company_id` (coupon_code, coupon_redemption, loyalty_order_points, loyalty_program,
/// loyalty_point_entry, pricing_rule, promo_bundle, promo_bundle_component), so all reads fence to
/// the caller's company under the RLS app role. `pool` is accepted for symmetry with the other
/// guarded composers and for future validated-write routes; the read surface itself uses the
/// services' own pools.
